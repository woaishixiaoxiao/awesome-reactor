...
std::string config_reader(const std::string& section, const);
...
std::string config_reader::trimLeft(const std::string& s) {
	...
	std::string::config_iterator beg = s.begin();
	while(beg != s.end()) {
		if(!isspace(*beg)) {
			break;
		}
...
std::string config_reader::trimLeft(const std::string& s) {
	size_t first_not_empty = s.length();
	
	std::string::config_iterator end = s.end();
	while(end != s.end()) {
		end--;
		if(!isspace(*beg)) {
			break;
	...
	return s.substr(0,first_not_empty);
}
